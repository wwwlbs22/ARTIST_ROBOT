...
from BADMUNDA import BDB_URI, LOGGER, TIME_ZONE
from BADMUNDA.bot_class import BAD
from BADMUNDA.database.chats_db import Chats

if BDB_URI:
    from BADMUNDA.plugins import bday_cinfo, bday_info
...
@BAD.on_message(command("remember"))
async def remember_me(c: BAD, m: Message):
    if not BDB_URI:
...
@BAD.on_message(command(["removebday","rmbday"]))
async def who_are_you_again(c: BAD, m: Message):
    if not BDB_URI:
        await m.reply_text("BDB_URI is not configured")
        return
    user = m.from_user.id
...
@BAD.on_message(command(["nextbdays","nbdays","birthdays","bdays"]))
async def who_is_next(c: BAD, m: Message):
    if not BDB_URI:
        await m.reply_text("BDB_URI is not configured")
        return
...
@BAD.on_message(command(["getbday","gbday","mybirthday","mybday"]))
async def cant_recall_it(c: BAD, m: Message):
    if not BDB_URI:
        await m.reply_text("BDB_URI is not configured")
        return
    user = m.from_user.id
...
@BAD.on_message(command(["settingbday","sbday"]))
async def chat_birthday_settings(c: BAD, m: Message):
    if not BDB_URI:
        await m.reply_text("BDB_URI is not configured")
        return
    if m.chat.type == ChatType.PRIVATE:
...
@BAD.on_callback_query(filters.regex("^switchh_"))
async def switch_on_off(c:BAD, q: CallbackQuery):
