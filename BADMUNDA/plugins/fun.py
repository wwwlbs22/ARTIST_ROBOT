...
from BADMUNDA.utils import extras
from BADMUNDA.utils.custom_filters import command
from BADMUNDA.utils.extras import NOWYES as NO
from BADMUNDA.utils.extras import YESWNO as YES

DEV_USERS = get_support_staff("dev")

@BAD.on_message(command("shout"))
...
@BAD.on_message(command("slap"))
async def fun_slap(c: BAD, m: Message):
    me = await c.get_me()
...
@BAD.on_message(command("insult"))
async def insult(c: BAD, m: Message):
...
@BAD.on_message(command("yes"))
async def yesw(c: BAD, m: Message):
    reply_text = m.reply_to_message.reply_text if m.reply_to_message else m.reply_text
    rtext = YES[0]
...
@BAD.on_message(command("no"))
async def now(c: BAD, m: Message):
    reply_text = m.reply_to_message.reply_text if m.reply_to_message else m.reply_text
    rtext = NO[0]
    await reply_text(rtext)
...
@BAD.on_message(command("bluetext"))
...
@BAD.on_message(command("weebify"))
