...
@BAD.on_callback_query(filters.regex("^formatting."))
async def get_formatting_info(c: BAD, q: CallbackQuery):
    cmd = q.data.split(".")[1]
...
Use this to make BAD feel a bit more customised! (only works in filters/notes)
...
@BAD.on_callback_query(filters.regex("^back."))
async def send_mod_help(_, q: CallbackQuery):
    await q.edit_message_caption(
        caption="""Formatting

BAD supports a large number of formatting options to make your messages more expressive. Take a look by clicking the buttons below!""",
