...
from BADMUNDA import LOGGER, MESSAGE_DUMP
from BADMUNDA.bot_class import BAD
from BADMUNDA.database.antispam_db import ANTISPAM_BANNED, GBan
from BADMUNDA.database.approve_db import Approve
from BADMUNDA.database.blacklist_db import Blacklist
from BADMUNDA.database.group_blacklist import BLACKLIST_CHATS
...
@BAD.on_message(filters.linked_channel)
async def antichanpin_cleanlinked(c: BAD, m: Message):
    try:
        msg_id = m.id
...
@BAD.on_message(filters.text & filters.group, group=5)
...
@BAD.on_message(filters.user(list(ANTISPAM_BANNED)) & filters.group)
async def gban_watcher(c: BAD, m: Message):
    from BADMUNDA import SUPPORT_GROUP
...
@BAD.on_message(filters.chat(BLACKLIST_CHATS))
async def bl_chats_watcher(c: BAD, m: Message):
    from BADMUNDA import SUPPORT_GROUP
