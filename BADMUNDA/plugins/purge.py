...
@BAD.on_message(command("purge") & admin_filter)
async def purge(c: BAD, m: Message):
...
@BAD.on_message(command("spurge") & admin_filter)
async def spurge(c: BAD, m: Message):
...
async def del_msg(c: BAD, m: Message):
