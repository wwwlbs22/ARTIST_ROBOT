...
from BADMUNDA.database.group_blacklist import GroupBlacklist
...
@BAD.on_message(command("blchat", dev_cmd=True))
async def blacklist_chat(c: BAD, m: Message):
...
async def unblacklist_chat(c: BAD, m: Message):
    if len(m.text.split()) >= 2:
        chat_ids = m.text.split()[1:]
