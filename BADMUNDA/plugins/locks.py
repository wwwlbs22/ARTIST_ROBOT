...
from BADMUNDA.database.locks_db import LOCKS
...
@BAD.on_message(command("locktypes"))
...
@BAD.on_message(command("lock") & restrict_filter)
async def lock_perm(c: BAD, m: Message):
...
@BAD.on_message(command("locks") & restrict_filter)
...
@BAD.on_message(command("unlock") & restrict_filter)
async def unlock_perm(c: BAD, m: Message):
    if len(m.text.split()) < 2:
...
async def delete_messages(c:BAD, m: Message):
    try:
        await m.delete()
        return
...
async def is_approved_user(c:BAD, m: Message):
...
@BAD.on_message(filters.group & ~filters.me,18)
async def lock_del_mess(c:BAD, m: Message):
    lock = LOCKS()
