...
@BAD.on_message(filters.command("gpinfo", prefixes="/"))
async def get_group_status(_, message: Message):
    ...
    try:
        group = await BAD.get_chat(group_username)
    except Exception as e:
        await message.reply(f"Error: {e}")
        return
    
    total_members = await BAD.get_chat_members_count(group.id)
...
@BAD.on_message(filters.command("status") & filters.group)
