...
from BADMUNDA.utils.caching import (ADMIN_CACHE, TEMP_ADMIN_CACHE_BLOCK,
                                  admin_cache_reload)
...
@BAD.on_message(command("adminlist"))
...
@BAD.on_message(command("zombies") & admin_filter)
async def zombie_clean(c: BAD, m: Message):
    zombie = 0
...
@BAD.on_message(command("admincache"))
...
@BAD.on_message(filters.regex(r"^(?i)@admin(s)?") & filters.group)
...
@BAD.on_message(command("fullpromote") & promote_filter)
async def fullpromote_usr(c: BAD, m: Message):
    global ADMIN_CACHE
    ...
    try:
        ...
        if m.chat.type in [ChatType.SUPERGROUP, ChatType.GROUP]:
            title = "BAD"  # Default fullpromote title
...
@BAD.on_message(command("promote") & promote_filter)
async def promote_usr(c: BAD, m: Message):
    global ADMIN_CACHE
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(
...
@BAD.on_message(command("demote") & promote_filter)
async def demote_usr(c: BAD, m: Message):
    global ADMIN_CACHE
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@BAD.on_message(command("invitelink"))
async def get_invitelink(c: BAD, m: Message):
...
@BAD.on_message(command("setgtitle") & admin_filter)
...
@BAD.on_message(command("setgdes") & admin_filter)
...
@BAD.on_message(command("title") & admin_filter)
async def set_user_title(c: BAD, m: Message):
    user = await m.chat.get_member(m.from_user.id)
...
@BAD.on_message(command("setgpic") & admin_filter)
async def setgpic(c: BAD, m: Message):
    user = await m.chat.get_member(m.from_user.id)
    if not user.privileges.can_change_info and user.status != CMS.OWNER:
        await m.reply_text(
            "You don't have enough permission to use this command!",
