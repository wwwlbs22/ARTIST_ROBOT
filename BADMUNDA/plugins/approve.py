...
@BAD.on_message(command("approve") & admin_filter)
async def approve_user(c: BAD, m: Message):
...
async def disapprove_user(c: BAD, m: Message):
    db = Approve(m.chat.id)
...
@BAD.on_message(command("approved") & admin_filter)
...
@BAD.on_message(command("approval") & filters.group)
async def check_approval(c: BAD, m: Message):
    db = Approve(m.chat.id)
...
@BAD.on_callback_query(filters.regex("^unapprove_all$"))
