...
from BADMUNDA.utils.custom_filters import admin_filter
...
async def tag_all_users(_,message): 
    ...
    if replied:
        ...
        usernum= 0
        usertxt = ""
        async for m in BAD.get_chat_members(message.chat.id): 
        ...
        async for m in BAD.get_chat_members(message.chat.id):       
            ...
            usertxt += f"\n⊚ [{m.user.first_name}](tg://user?id={m.user.id})\n"
            if usernum == 5:
                await BAD.send_message(message.chat.id,f'{text}\n{usertxt}')
                await asyncio.sleep(2)
                usernum = 0
                usertxt = ""                          
        try :
...
@BAD.on_message(filters.command("tagoff") & ~filters.private & admin_filter)
