...
async def tgs_to_gif(file, tgs=False, video=False):
    if tgs:
        cmd = f"lottie_convert.py '{file}' 'BAD_satoru.gif'"
    elif video:
        cmd = f"ffmpeg -i '{file}' -c copy 'BAD_satoru.gif'"
    await runcmd(cmd)
    os.remove(file)
    return 'BAD_satoru.gif'

async def webm_to_gif(file):
    cmd = f"ffmpeg -i '{file}' 'BAD.gif'"
    await runcmd(cmd)
    os.remove(file)
    return "BAD.gif"
        
async def Vsticker(c: BAD, file: Message):
    if file.animation:
        file = file.animation
    elif file.video:
        file = file.video
...
async def upload_document(
    client: BAD, file_path: str, chat_id: int
...
def toimage(image, filename=None, is_direc=False):
    filename = filename if filename else "BAD.jpg"
    if is_direc:
...
def tosticker(response, filename=None, is_direc=False):
    filename = filename if filename else "BAD.webp"
    if is_direc:
